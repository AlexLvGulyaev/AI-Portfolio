...
| 2026-08-29 | 1.8 | Консоль «Источники и синхронизация» доведена до текущего вида: слияние легаси-консоли «Синхронизация» с источниками, тулбар-стрип корпуса, backfill approved-полей и событий для 11 легаси-источников; зафиксирована политика состава KB (§5.1, решение владельца) |
| 2026-08-29 | 1.9 | Три условия допуска: привязка «источник ↔ карточка» (гвард POST + FK NOT NULL/RESTRICT, миграция 016), «один репозиторий = один источник» (unique-индекс 017 + 409), селектор свободных не-дочерних карточек (миграция 018); namespace-гвард KB_REPO_OWNER с live-пробой, fail-closed при недоступности GitHub (503). Гварды проверены в production, E1+E2 17/17 |
| 2026-08-29 | 1.10 | Селектор репозиториев владельца вместо свободного URL (GET /admin/knowledge-base/github-repos, подключённые скрыты, архив помечен); GitHub недоступен → 503 + «Повторить загрузку». Unit 38/38, E1+E2 17/17 |
| 2026-08-29 | 1.11 | Синхронизация KB: живой прогрессбар + single-flight (409 sync_already_running, миграция 019, re-attach прогресса после перезагрузки страницы). Миграция проверена на pg_dump-копии; unit 42/42; прод: 3 висячие задачи закрыты, 0 running |
| 2026-08-29 | 1.12 | Стадийная видимость: retrieval-гвард скрытых проектов в ChatOrchestrator (PortfolioRegistry.hidden_repos, Chroma $nin для глобального поиска и fallback); unit 44/44, живые проверки на проде |
| 2026-08-29 | 1.13 | Динамические инвариантные базы E1/E2 (17/17 на составе 13 источников) + канал проверки владельцем POST /admin/chat-preview (include_hidden=True, отдельный кеш); live: публичный чат — отказ без чанков скрытого репозитория, preview — содержательный ответ |
