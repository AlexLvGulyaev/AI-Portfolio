...
|------|--------|-----------|
| 2026-07-14 | 0.9 | Первый технический черновик на основе Assistant Flow и Review Flow (9 рабочих пространств) |
| 2026-07-15 | 1.0 | Актуализация под продуктовую концепцию: 3 рабочих пространства, единый env-token, отказ от RBAC/JWT |
| 2026-07-18 | 1.1 | Управление LLM-провайдерами через `/admin/ai-providers`; параметры провайдеров — SOT в БД |
| 2026-07-18 | 1.2 | Execution Tracing развёрнут: `ExecutionSession`/`ExecutionStep`, миграции 007–009, `/admin/execution-sessions`, консоль «Логи»; актуализирована навигация админки |
| 2026-07-19 | 1.3 | Аудит входа в админку и посещений сайта: `/admin/login`, `/track-visit`, миграция 010, вкладка «Аудит» |
| 2026-07-19 | 1.4 | Логирование упрощено: `execution_sessions` — единый SOT chat pipeline; `visitor_id`/`client_ip`/`user_agent` (миграция 011); вкладки UI разделены |
| 2026-07-19 | 1.5 | Страница «Диалоги» переработана по образцу Assistant Flow Memory Console; расширены `GET /admin/conversations`, заменён `ConversationsPage.tsx` |
| 2026-07-19 | 1.6 | §5 дополнен планом GitHub Sync (Этап 11.11 IMPLEMENTATION_PLAN.md) |
| 2026-07-19 | 1.7 | GitHub Sync развёрнут: 7 источников, 192 документа, 5400 чанков; ChromaDB вынесена в отдельный HTTP-сервис `ai-portfolio-chroma`; §5 переработан под фактическую архитектуру |
| 2026-08-29 | 1.8 | Консоль «Источники и синхронизация» доведена до текущего вида; зафиксирована политика состава KB (§5.1) |
| 2026-08-29 | 1.9 | Три условия допуска: привязка «источник ↔ карточка» (миграция 016), «один репозиторий = один источник» (017), селектор карточек (018); namespace-гвард fail-closed (503) |
| 2026-08-29 | 1.10 | Селектор репозиториев владельца вместо свободного URL; GitHub недоступен → 503 + «Повторить загрузку» |
| 2026-08-29 | 1.11 | Синхронизация: живой прогрессбар + single-flight (409, миграция 019, re-attach прогресса) |
| 2026-08-29 | 1.12 | Стадийная видимость: retrieval-гвард скрытых проектов (`$nin` для глобального поиска и fallback) |
| 2026-08-29 | 1.13 | Динамические инвариантные базы E1/E2 + канал проверки `POST /admin/chat-preview` |