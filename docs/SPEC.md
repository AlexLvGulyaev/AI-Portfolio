...
**Реализация пробы (03.09.2026):** первый шаг сценария реализован как Morphic-проба — на главной действует сцена подбора кейса по описанию задачи: посетитель формулирует задачу, платформа возвращает **до 3 релевантных кейсов с одной строкой «почему подходит»** (`POST /case-match`; подбор grounded — retrieval по документам KB + реестр видимых карточек, объяснение генерируется по документам без обещаний статусов и цен). Сцена включается флагом `MORPHIC_ENABLED` (откат — флагом). Полный UX-контур (сессии подбора, визард, диалоговый поиск как главный пресейл-механизм) — отдельное направление (см. [`docs/ROADMAP.md`](ROADMAP.md) §5).
...
> Сценарий 6 (Morphic-сцена): первый шаг реализован пробой 03.09.2026 — подбор кейса по описанию задачи (`POST /case-match`, до 3 кейсов с объяснением применимости, флаг `MORPHIC_ENABLED`); полный диалоговый контур — целевое состояние следующего этапа (границы в описании Сценария 6, развитие — [`docs/ROADMAP.md`](ROADMAP.md) §5).

---

## 🗺️ 4. Информационная архитектура
...
## ⚙️ 6. Нефункциональные требования
...
| Требование | Описание |
|-------------|----------|
| Визуальный стиль | Инженерный минимализм, фирменные материалы проекта |
...
Полный список технических вопросов — отработан в ходе проекта и закрыт (см. `docs/IMPLEMENTATION_PLAN.md`, раздел открытых вопросов)

---

## 🎛️ 8. Административная консоль
...
| Morphic | Проба реализована (03.09.2026): подбор кейса по описанию задачи на главной — `POST /case-match`, до 3 кейсов с why-объяснением по документам, флаг `MORPHIC_ENABLED`; полный UX-контур (сессии, визард) — отдельное направление (см. Сценарий 6 и [`docs/ROADMAP.md`](ROADMAP.md) §5). |
...
| 2026-07-12 | 1.1 | Классификация открытых вопросов, закрытие категории 2, перенос категории 3 в backlog |
| 2026-07-12 | 1.2 | Закрытие всех открытых вопросов решениями владельца продукта |
| 2026-08-25 | 2.0 | Актуализация под dual-theme главную: 13 карточек (12 проектов + Prompt Review placeholder), featured/archive по `display_order` |
| 2026-08-25 | 2.1 | Уточнён состав 12 завершённых проектов; исправлена путаница Telegram Support Bot / Telegram Intake Bot; Prompt Review — placeholder без Light/Dark-пары |
| 2026-08-25 | 2.2 | Зафиксирован единый dual-theme шаблон лендинга кейса (`AIP Case Landing Page Standard v1.1`, эталон — AI Curator) |
| 2026-08-27 | 2.3 | As-built baseline под фактический продукт на 27.08: 13 полноценных кейсов, главная = витрина всех проектов; эталон шаблона — `/cases/ai-curator.html`; состав KB — через admission gate; добавлен Morphic-сценарий 6. Продукт не перепроектирован |
| 2026-09-03 | 2.4 | Morphic-проба реализована (Сценарий 6, `POST /case-match`, флаг `MORPHIC_ENABLED`); полный UX-контур — отдельное направление (`docs/ROADMAP.md`) |