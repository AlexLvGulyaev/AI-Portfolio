...
Согласно актуальному составу `ProjectCard`, на сайте представлены **13 управляемых карточек**. Из них **12 — завершённые портфельные проекты**, а тринадцатая — **Prompt Review placeholder / карточка-анонс**.

| № | Кейс | Статус | Примечание |
|---|------|--------|------------|
| 1 | AI Curator | Production-ready | Featured (`display_order` 1) |
...
| 13 | Prompt Review | Placeholder / анонс | Нет Light/Dark-пары, страница и материалы в разработке |

**Исключённые из публичной двенадцатки карточки:** `telegram-ai-gateway`, `competitor-monitor-ai` — присутствуют в `ProjectCard`, но не отображаются на главной (`is_visible = false` / вне первых 13 по `display_order`).
...
| Единый шаблон страницы кейса | Все лендинги кейсов следуют `AIP Case Landing Page Standard v1.1`; эталон — AI Curator (`/cases/ai-curator-template-preview.html`) |
...
| **Портфолио** | 13 управляемых карточек: 12 завершённых кейсов APL + Prompt Review placeholder |
...
| Портфолио | Все 13 управляемых карточек представлены с согласованным составом информации; Prompt Review обозначен как placeholder |
...
| Портфолио: 13 управляемых карточек | PROJECT_STATE.md — Owner Decisions (SOT) |
...
| Состав портфолио на старте | 13 управляемых карточек: 12 завершённых проектов + Prompt Review placeholder. |
| Логика главной страницы | Все видимые карточки выводятся на главной. `display_order` 1..3 — флагманы, 4+ — архив. `show_on_homepage` не используется для фильтрации. |
| Prompt Review | Карточка-анонс на позиции 13; полноценная страница и материалы появятся позже. |
