...
| 8 | HR Assistant — LoRA Fine-Tuning | Production-ready |
| 5 | Prompt Review | Production-ready |
| 6 | Telegram AI Gateway | Production-ready |
| 7 | Competitor Monitor AI | — |

**Порядок в каталоге:** по полю `display_order` (управляется в административной консоли).

**Карточки на главной странице:**

На главной странице отображается подмножество кейсов, выбранное через поле `show_on_homepage`. Значение `0` означает «не отображать на главной», значения `1..4` определяют порядок отображения слева направо. Конкретный набор и порядок карточек на главной управляются в административной консоли.
...
Публичный frontend получает карточки проектов через единый **read-only API backend** и отображает их в каталоге портфолио и на главной странице без хранения канонических данных в статическом HTML. Каталог портфолио сортирует карточки по `display_order`, главная страница фильтрует по `show_on_homepage > 0` и сортирует по `show_on_homepage`.
