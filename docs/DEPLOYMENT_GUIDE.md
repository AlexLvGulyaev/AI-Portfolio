...
**Проект:** ai-portfolio
**Версия:** 1.2 (v1.1 прошла Deployment Validation 02.09.2026 в dind-стенде, 3 прогона fresh-from-zero)
**Дата:** 2026-09-02
...
Реестр карточек проектов заполняется миграциями автоматически:
`003` сеет стартовый набор, `013` приводит реестр к финальному составу
(13 видимых карточек портфеля + 2 скрытые legacy-записи) — после него
витрина (`GET /project-cards`) наполнена без ручных действий.
...
3. Настроить **draft-правила допуска** источника (include/exclude паттерны — рабочий экземпляр использует `README.md` + публичные docs-документы, внутренние документы исключены): Preview состава по умолчанию включает 0 файлов, пока правила не заданы.
4. Построить **Preview состава** → проверить состав файлов → **Одобрить** (одобряется последний ready-preview; после смены правил нужен новый Preview).
5. **Синхронизировать** — прогрессбар в консоли; по завершении в консоли «Документы» видны документы и чанки.

> Для включения документации самой платформы AI Portfolio (источник
> `AlexLvGulyaev/AI-Portfolio`) сначала создаётся мета-карточка платформы
> (категория `meta`, флаг `is_meta`, скрыта с витрины) — «Проекты → создать
> карточку», затем источник привязывается к ней. Миграция создаёт только
> колонку-флаг `is_meta`; саму карточку администратор создаёт после деплоя.
...
### 8.2.1 Политика кеша статики (nginx)

`src/nginx.conf` делит статику по изменяемости (находка 03.09.2026 —
CTA виджета не был виден возвращающимся зрителям из-за годового
`immutable`-кеша JS):

| Тип | Политика | Обоснование |
|-----|----------|-------------|
| `.png/.jpg/.gif/.ico/.svg/.woff*/.ttf/.eot` | `expires 1y` + `public, immutable` | Картинки/шрифты между деплоями не меняются |
| `.css/.js` | `Cache-Control: no-cache` | Ревалидация по ETag (304 без тела); любой деплой JS/CSS виден всем сразу |

Ссылки на скрипты на страницах витрины версионированы параметром
(`js/chat-widget.js?v=YYYYMMDD`): параметр обновляется при изменении
файла и вытесняет кеши, уже сохранённые под старой политикой `immutable`.
После смены версии в ссылках новые копии попадают под `no-cache` и далее
ревалидируются автоматически.

### 8.2.2 Rate limit публичных API (nginx)

Публичные API-эндпойнты (`/chat`, `/project-cards`, `/track-visit`,
`/track-event`, `/document-fragment`, `/case-match`) ограничены nginx'ом
фронтенда: **10 запросов/минуту на IP клиента**, burst 20 (гасит веер
запросов при загрузке страницы), превышение — `429`. `/health` и
`/api/admin/*` не ограничиваются.

Edge-прокси — Traefik (TLS-терминация в сети `n8n_default`): без
`set_real_ip_from` + `real_ip_header X-Forwarded-For` nginx видел бы IP
Traefik, и лимит был бы общим на всех зрителей. Доверенный диапазон в
`nginx.conf` (`172.18.0.0/16`) — подсеть `n8n_default`; при переносе в
другую сеть обновить его под фактическую подсеть edge.

Проверка лимита: 21+ подряд запрос к `/project-cards` — первые 20
проходят (burst), далее `429`; через ~6 секунд простоя — снова `200`.

### 8.2.3 SSE-стрим чата (nginx, `/chat/stream`)

Потоковый чат идёт через `POST /chat/stream` (см.
[`API_CONTRACT.md`](API_CONTRACT.md) §2a). В `src/nginx.conf` для него
выделен отдельный `location = /chat/stream` (exact-матч сильнее
префиксного `/chat`) с двумя обязательными директивами:

- `proxy_buffering off` — nginx копит проксированный ответ в буферы;
  с буферизацией все дельты доходили бы пачкой после генерации и
  TTFT-эффект стрима терялся бы полностью;
- `proxy_cache off` — стрим не кешируется.

Backend дополнительно шлёт `X-Accel-Buffering: no` в заголовках каждого
стрим-ответа (per-response отключение буферизации — защита на случай
конфигурации nginx без location-директивы). Rate limit тот же, что у
`/chat` (10 req/min, burst 20), `429` до открытия потока.

Проверка (после `docker compose up` и загрузки страницы): отправить
сообщение в чат-виджете и убедиться, что текст появляется по мере
генерации, а не пачкой в конце; либо curl'ом:

```bash
curl -N -X POST https://<домен>/chat/stream \
  -H 'Content-Type: application/json' \
  -d '{"message":"Какие кейсы в портфолио?"}' | head -20
```

Ожидаемо: события `data: {"type": "start"}`, затем `delta`-события по
мере генерации, затем `final` и `data: [DONE]`. Если всё приходит одним
комком после паузы — буферизация не отключена (проверить `proxy_buffering
off` в `location = /chat/stream` и заголовок `X-Accel-Buffering`).
...
| Чат ассистента отвечает 503 «временно недоступен» | В `system_prompts` нет активной строки: миграция 025 сеет вшитый базлайн на чистом развёртывании; если таблица очищена — активировать версию в «Система → AI-настройки» |
...
| 2026-09-02 | 1.1 | По результатам Deployment Validation (dind, fresh-from-zero ×3): уточнено описание сида реестра (003 стартовый набор → 013 финальный состав, 13 видимых + 2 legacy-скрытых); Шаг 7 дополнен настройкой draft-правил до Preview/Одобрить и созданием мета-карточки платформы для источника `AlexLvGulyaev/AI-Portfolio` |
| 2026-09-03 | 1.2 | §8.2.2: rate limit публичных API nginx'ом (10 req/min на IP, burst 20, 429) — enforcement найденного незадействованного поля конфига; real_ip по X-Forwarded-For за Traefik, доверенная подсеть n8n_default. Валидировано на production (burst-тест + real_ip по логу) |
| 2026-09-10 | 1.3 | §8.2.3: SSE-стрим `/chat/stream` — nginx `location` exact-матч с `proxy_buffering off` / `proxy_cache off`, `X-Accel-Buffering: no` от backend; curl-проверка дельт по мере генерации. Валидация запуском 09.09 на production: первая дельта 1559 мс при полном ответе 4738 мс, инкрементальность подтверждена |