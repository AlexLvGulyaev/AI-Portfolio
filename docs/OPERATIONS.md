...
Что видит зритель (UX деградации, приёмочное ревью):
...
| 2026-09-03 | 1.1 | §4: каналы деградации развёрнуты в таблицу — failover LLM (триггер, журнал), деградация retrieval без 500 (приёмочное ревью), UX-тексты отказов; схема деградации запроса (mermaid); признак `retrieval_error` в шаге `rag_search` |