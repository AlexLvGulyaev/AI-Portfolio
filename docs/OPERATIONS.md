...
Каналы деградации системы по порядку: основной LLM → резервный LLM (настроен); retrieval-канал эмбеддингов — резерв не реализован (осознанное ограничение, [`docs/ROADMAP.md`](ROADMAP.md) §4 п.6). При недоступности retrieval ассистент деградирует к честному отказу — это ожидаемое поведение контура, а не сбой.
