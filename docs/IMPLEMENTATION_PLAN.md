...
**Public Frontend:** остаётся без изменений (vanilla HTML + CSS + JavaScript)

**Admin Frontend:** отдельный React + TypeScript + Vite модуль внутри проекта

**Маршрут:** `/admin/`

**Backend:** единый, расширяется admin endpoints

### Детали

Полная архитектура административной консоли зафиксирована в `docs/ADMIN_CONSOLE_ARCHITECTURE.md`.

### Примечание

Этот этап сознательно не входит в первую публикацию проекта. Он будет реализован после Deployment Validation и подготовки публичного репозитория.
