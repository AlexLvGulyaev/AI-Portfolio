...
**Примечание по Knowledge Base v1:** ручная синхронизация в Stage 4 перестраивает ChromaDB из локального файла `knowledge_base/knowledge.json` и поля `knowledge_content` карточек проектов. GitHub остаётся Source of Truth для проектной документации; использование `knowledge.json` — временное техническое решение до реализации GitHub Sync.
...
- CRUD управляемых карточек проектов (`ProjectCard`) — backend + frontend.
- CRUD источников Knowledge Base (`KnowledgeSource`) — backend + frontend.
- ручная синхронизация → ChromaDB через `POST /admin/knowledge-base/sync`. В v1 синхронизация использует локальный `knowledge_base/knowledge.json` и `knowledge_content` карточек как временный источник до реализации GitHub Sync; GitHub остаётся архитектурным SOT проектной документации.
- панель статуса ChromaDB.
...
9. ~~Реализация административной консоли (Stage 1–4)~~ — ✅ Выполнено. Backend Foundation, Admin Frontend Foundation, Infrastructure Integration и три рабочих пространства реализованы и развёрнуты.
10. **Deployment Validation** — ⏳ Будет проведён по решению владельца проекта перед финальной публикацией.
...
| 2026-07-15 | 1.10 | Stage 4 и 11.5 отмечены завершёнными. Устранены противоречия в статусах административной консоли. Зафиксировано временное использование `knowledge_base/knowledge.json` до реализации GitHub Sync. |
