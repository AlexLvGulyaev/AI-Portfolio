...
class ChatOrchestrator:
    ...
    def _is_refusal(answer: str) -> bool:
        ...
        Формулировки заданы системным промптом (правило 2) и его
        контекстными вариациями («нет информации», «нет такой аббревиатуры»
        — прод-наблюдение 04.09.2026). Такие ответы не кешируются: LLM
        может отказать стохастически при релевантном контексте, и кеш
        зафиксировал бы неудачный исход для всех новых сессий; они же
        отдаются без источников (панель источников под отказом вводит
        в заблуждение — решение владельца 04.09.2026).
        """
        low = (answer or "").lower()
        return any(
            marker in low
