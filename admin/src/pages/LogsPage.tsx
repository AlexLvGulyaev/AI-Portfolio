...
function pickTextByPaths(detailsPool: Record<string, unknown>[], paths: string[]): string | null {
  for (let i = detailsPool.length - 1; i >= 0; i--) {
...
function buildRagFallback(detailsPool: Record<string, unknown>[]): string | null {
  for (let i = detailsPool.length - 1; i >= 0; i--) {
    const d = detailsPool[i];
    const hasAny = d.retrieved_count != null || d.context_chars != null || d.fallback_reason != null;
    if (!hasAny) continue;
...
function pickRoute(rows: LogItem[]): RouteFilter {
  ...
  for (let i = ordered.length - 1; i >= 0; i--) {
    const mr = (ordered[i].modality_route || "").trim().toLowerCase();
    if (["text", "rag", "image", "audio", "document"].includes(mr)) {
  ...
  const vals = ordered
    .flatMap((r) => [
      String(r.route || ""),
      String(r.mode || ""),
      String(asRecord(r.details)?.route || ""),
      String(asRecord(r.details)?.mode || ""),
    ])
...
function pickRouteKey(rows: LogItem[]): string {
  const route = pickRoute(rows);
  if (route === "image") return "image_generation";
...
function buildSessions(rows: LogItem[]): SessionView[] {
  ...
  for (const [id, chunk] of grouped) {
    ...
    const detailsPool = ordered.map((r) => asRecord(r.details)).filter((d): d is Record<string, unknown> => d !== null);
    const route = pickRoute(ordered);
    const routeKey = pickRouteKey(ordered);
    const status = String(latest.status || "—");
    const providerModel = pickProviderModel(detailsPool);
    const userInput = pickText(detailsPool, ["query", "user_text", "prompt", "input_text", "text"]);
    ...
    const tsList = ordered.map((r) => toTs(r.created_at)).filter((t): t is number => t != null);
    ...
    const pipelineSummary = ordered.map((r) => stageToActionRu(r.stage)).join(" → ");
    ...
    out.push({
      ...
      routeKey,
      status,
      preview: userInput || assistantOutputBase || ragAnswer || detailsJsonPreview(latest.details),
      providerModel,
      wallDurationMs,
      maxStageLatencyMs,
      avgStageLatencyMs,
...
function filterSessions(
  ...
  return sessions.filter((s) => {
    ...
    const searchHay = [
      ...
      s.pipelineSummary,
      ...s.rows.map(
        (r) => `${r.stage} ${stageToActionRu(r.stage)} ${detailsJsonPreview(r.details)}`
      ),
    ]
      .join(" ")
      .toLowerCase();
...
function detailValue(session: SessionView, side: "left" | "right"): string {
  if (session.routeKey === "audio") {
    return side === "left" ? session.userInput ?? "—" : session.assistantOutput ?? "н/д";
  }
  if (session.routeKey === "image_generation") {
    return side === "left"
      ? session.userInput ?? "—"
      : session.assistantOutput ?? session.ragAnswer ?? "н/д";
  }
  if (session.routeKey === "rag") {
    return side === "left"
      ? session.userInput ?? "—"
      : session.ragAnswer ?? session.ragFallback ?? session.assistantOutput ?? "н/д";
  }
  return side === "left" ? session.userInput ?? "—" : session.assistantOutput ?? "н/д";
}
...
export function LogsPage() {
  ...
  const filtered = useMemo(
    () => filterSessions(sessions, routeFilter, statusFilter, search, windowLabelToMs(windowLabel)),
  ...
  const selected =
    pageSessions.find((s) => s.id === selectedId) ??
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      const curIdx = selectedId ? filtered.findIndex((s) => s.id === selectedId) : pageIndex * PAGE_SIZE;
      if (curIdx < 0) return;
      const nextIdx =
        e.key === "ArrowDown"
  ...
  return (
    <div className="page logs-page">
      ...
      <p className="page__lead logs-lead">
        ЖУРНАЛ EXECUTION-СЕССИЙ · <code>/api/admin/logs/recent</code> · время: МСК
      </p>
          ...
          <section className="logs-left card">
            ...
            <div className="logs-list" ref={listRef}>
              {loading && items.length === 0 ? (
                ...
                <EmptyState message="За выбранный период события не найдены." />
              ) : filtered.length === 0 ? (
                ...
                pageSessions.map((s) => (
                  ...
                  >
                    ...
                    <div className="logs-item__row logs-item__meta muted">
                      ...
                      <span title="Общая длительность">{formatDurationMs(s.wallDurationMs)}</span>
                      <span title="Макс. длительность этапа">
            ...
            {!selected ? (
              ...
              <div className="logs-detail">
                <div className="logs-detail__head">
                  ...
                  <span className={`logs-status logs-status--${normalizeStatus(selected.status)}`}>
                ...
                <div className="logs-detail-grid page__mt logs-detail-grid--dense">
                  <div className="logs-detail-block">
                    ...
                    <details className="logs-details-inline">
                      <summary className="log-details__summary">{detailLabels(selected.routeKey).left}</summary>
                      <pre className="log-details__json mono">{detailValue(selected, "left")}</pre>
                    </details>
                  </div>
                  <div className="logs-detail-block">
                    <h3 className="logs-detail-block__title">ЧТО ОТВЕТИЛА СИСТЕМА</h3>
                    <pre className="logs-pre mono">{detailValue(selected, "right")}</pre>
                    <details className="logs-details-inline">
                      <summary className="log-details__summary">{detailLabels(selected.routeKey).right}</summary>
                      <pre className="log-details__json mono">{detailValue(selected, "right")}</pre>
                    </details>
                  </div>
                </div>
                ...
                <div className="logs-timeline">
                  {selected.rows.map((row, i) => {
                    ...
                    const label = stageToActionRu(row.stage);
                    return (
                      <div
                        key={`${stageRaw}-${i}`}
                      ...
                      >
                        <div className="logs-stage__top">
                          ...
                          <span className={`logs-status logs-status--${normalizeStatus(row.status)}`}>
