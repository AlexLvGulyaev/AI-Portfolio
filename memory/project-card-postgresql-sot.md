...
| Правило | Следствие |
|---------|-----------|
| ProjectCard в PostgreSQL — единственный SOT карточек | Все канонические данные карточек (заголовок, описание, категория, видимость, порядок в портфолио, порядок на главной) живут в БД |
...
| Public frontend получает карточки через read-only API backend | Vanilla frontend загружает карточки с backend. Каталог портфолио сортирует по `display_order`, главная страница фильтрует и сортирует по `show_on_homepage` |
...
| `display_order` управляет порядком в каталоге | Все видимые карточки отображаются на `portfolio.html` в порядке `display_order` |
| `show_on_homepage` управляет отображением на главной | `0` — не отображать на `index.html`; `1..4` — порядок отображения на главной слева направо |

## Границы

- Решение относится **только к карточкам проектов в каталоге портфолио и на главной странице**.
...
- **Каталог портфолио и главная страница используют один read-only endpoint, но разные поля сортировки:** `display_order` для `portfolio.html`, `show_on_homepage` для `index.html`.
